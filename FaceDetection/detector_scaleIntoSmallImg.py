...
def scanImgAtScale(model, image, scale):
    ...
    SEARCH_WIN_WIDTH  = TRAINING_IMG_WIDTH
    SEARCH_WIN_HEIGHT = TRAINING_IMG_HEIGHT
...
def scanImgOverScale(image):
    ...
    for scale in numpy.arange(0.5, 0.6, 0.1):
        img = resizeImg(image, scale)
...
def optimalRectangle(rectangles, overlap_threshold):
    ...
    for i in xrange(numRec):
        ...
        for j in xrange(i, numRec):
            ...
            overlapArea = max(min(x1+w1, x2+w2) - max(x1, x2), 0) * max(min(y1+h1, y2+h2) - max(y1 ,y2), 0)
            totalArea   = (area_1 + area_2) - overlapArea
